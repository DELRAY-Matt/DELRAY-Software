...
class StartSliceJob(Job):
    ...
    # BigRep function!!
    # Sort and assembly the object with respect to the given printer.
    def reAssemblyOneAtATimeObjects(self, object_groups, isStudio):
        ...
        def isDisjoint(box0, box1, isStudio):
            if isStudio:
                return box0.minimum.x > box1.maximum.x or box1.minimum.x > box0.maximum.x
            else:
                return box0.minimum.z > box1.maximum.z or box1.minimum.z > box0.maximum.z

        object_groups = [[item] for item in flatten(object_groups)]

        if len(object_groups) > 0:
            if isStudio:
                object_groups.sort(key=lambda node: node[0].getBoundingBox().left, reverse=True)
            else:
                object_groups.sort(key=lambda node: node[0].getBoundingBox().back, reverse=True)

            union_item = object_groups[0]
            union_box = union_item[0].getBoundingBox()
            for item in object_groups[1:]:
                new_box = item[0].getBoundingBox()
                if isDisjoint(union_box, new_box, isStudio):
                    union_item = item
                    union_box = new_box
                else:
                    union_box = union_box + new_box
                    union_item.extend(item)
                    item.clear()

        object_groups = [item for item in object_groups if item]
        return object_groups

    # BigRep function!!
    def isTooBig_BuildVolumeForBigRepPro(self, object_groups, new_pro):
        def isOutside(point, box):
            # we don't need to test y because it is already tested before.
            return point[0] < box.minimum.x or point[0] > box.maximum.x or point[2] < box.minimum.z or point[2] > box.maximum.z

        def isInside(smallBox, bigBox0):
            # we don't need to test y because it is already tested before.
            return smallBox.minimum.x >= bigBox0.minimum.x and smallBox.maximum.x <= bigBox0.maximum.x and smallBox.minimum.z >= bigBox0.minimum.z and smallBox.maximum.z <= bigBox0.maximum.z

        def is_borderpoint_outside(box0, box1, v_box0, v_box1):
            if v_box0[1] <= box0.maximum.y:
                return False

            z_gap = box0.maximum.y - v_box0[1]
            direction = v_box1 - v_box0
            direction = direction / numpy.sqrt(numpy.sum(direction**2))
            if abs(direction[1]) < 0.0001:
                return False

            length = 1.0 / (z_gap * direction[1]) # 1 / scalar_product((0,z_gap,0), direction)
            edge_intersection = v_box0 + length * direction

            return isOutside(edge_intersection, box1)


        if new_pro:
            # box0 = AxisAlignedBox(Vector(0.0, 0.0, 0.0), Vector(975.0, 880, 973))
            # box1 = AxisAlignedBox(Vector(28.0, 28.0, 0.0), Vector(970.0, 835, 985))
            box0 = AxisAlignedBox(Vector(-487.5, 0.0, -486.5), Vector(487.5, 879.5, 486.5)) # 879.5 just to be sure!
            box1 = AxisAlignedBox(Vector(-459.5, 0.0, -348.5), Vector(482.5, 985.0, 461.5))
        else:
            # box0 = AxisAlignedBox(Vector(0.0, 0.0, 0.0), Vector(970.0, 935, 880))
            # box1 = AxisAlignedBox(Vector(58.0, 28.0, 0.0), Vector(950.0, 865, 950))
            box0 = AxisAlignedBox(Vector(-485.0, 0.0, -467.5), Vector(485.0, 879, 467.5)) # 879.5 just to be sure!
            box1 = AxisAlignedBox(Vector(-427.0, 0.0, -397.5), Vector(465.0, 950.0, 439.5))

        for group in object_groups:
            for node in group:
                nodeBox = node.getBoundingBox()
                if nodeBox.maximum.y <= box0.maximum.y:
                    continue

                # Check if nodeBox is completely inside box1
                if isInside(nodeBox, box1):
                    continue

                # Now we are in the interesting case.
                transformed_mesh = node.getMeshData().getTransformed(node.getWorldTransformation())
                if transformed_mesh.getIndices():
                    indices = transformed_mesh.getIndices()
                    vertices = transformed_mesh.getVertices()
                    for i, vertexIndex in enumerate(transformed_mesh.getIndices()):
                        vertex = transformed_mesh.getVertex(vertexIndex)
                        if vertex[1] > box0.maximum.y:
                            if isOutside(vertex, box1):
                                return True
                            else:
                                baseI = int(i / 3)
                                offset = i - baseI
                                    
                                if is_borderpoint_outside(box0, box1, vertices[indices[baseI + ((offset + 1) % 3)]], vertex):
                                    return True
                                elif is_borderpoint_outside(box0, box1, vertices[indices[baseI + ((offset -1) % 3)]], vertex):
                                    return True
                else:
                    vertices = transformed_mesh.getVertices()
                    for i, vertex in enumerate(vertices):
                        if vertex[1] > box0.maximum.y:
                            if isOutside(vertex, box1):
                                ...
                                if is_borderpoint_outside(box0, box1, vertices[baseI + ((offset + 1) % 3)], vertex):
                                    return True
                                elif is_borderpoint_outside(box0, box1, vertices[baseI + ((offset -1) % 3)], vertex):
                                    return True
    ...
    def run(self) -> None:
        ...
        with self._scene.getSceneLock():
            ...
            if stack.getProperty("print_sequence", "value") == "one_at_a_time":
                ...
                # BigRep Code
                isStudio = CuraApplication.getInstance().getMachineManager().activeMachine.definition.getId() == 'bigrep_studio'or CuraApplication.getInstance().getMachineManager().activeMachine.definition.getId() == "bigrep_studio_g2"
                object_groups = self.reAssemblyOneAtATimeObjects(object_groups, isStudio)
            ...
            # BigRep Code
            # Check build volume for BigRep Pro
            printer_id = CuraApplication.getInstance().getMachineManager().activeMachine.definition.getId()
            if printer_id.startswith('bigrep_pro'):
                new_pro = (printer_id != 'bigrep_pro')
                if self.isTooBig_BuildVolumeForBigRepPro(object_groups, new_pro):
                    object_groups.clear()
