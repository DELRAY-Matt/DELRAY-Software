...
regexESearch = re.compile(r"EX=IC\((-?\d.\d*)")
...
class FlavorParser:
    ...
    def _createPolygon(self, layer_thickness: float, path: List[List[Union[float, int]]], extruder_offsets: List[float]) -> bool:
        ...
        line_widths[:, 0] = 1.2  # Just a guess # BigRep Code
        ...
        for point in path:
            ...
            if i > 0:
                ...
                if point[5] in [LayerPolygon.MoveCombingType, LayerPolygon.MoveRetractionType]:
                    line_widths[i - 1] = 0.6 # BigRep Code
    ...
    def _calculateLineWidth(self, current_point: Position, previous_point: Position, current_extrusion: float, previous_extrusion: float, layer_thickness: float) -> float:
        ...
        if line_width > 10.2: # BigRep Code
            return 1.2 # BigRep Code
        return line_width
        ...
        if func is not None:
            ...
            for item in s[1:]:
                ...
                if item[0] == "F":
                    f = float(item[1:]) / 60
                if item[0] == "E":
                    eSearch = regexESearch.search(line)
                    if eSearch:
                        e = float(eSearch.group(1))                
                        self._is_absolute_extrusion = False
                    else:
                        e = float(item[1:])
